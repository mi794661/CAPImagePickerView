...
- (void)viewDidLoad {
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = [UIColor colorWithRed:(60/255.0) green:(150/255.0) blue:(230/255.0) alpha:1.0];
    self.oKButtonTitleColorDisabled = [UIColor colorWithRed:(60/255.0) green:(150/255.0) blue:(230/255.0) alpha:0.5];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<TZImagePickerControllerDelegate>)delegate {
    ...
    if (self) {
        ...
        if (![[TZImageManager manager] authorizationStatusAuthorized]) {
            ...
            _tipLable.text = [NSString stringWithFormat:@"请在\"设置-%@\"中允许%@对照片的访问", appName, appName];
